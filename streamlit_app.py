...
for date, row in filtered.iterrows():
    ...
    if curr_label != prev_label:
        if prev_label is not None:
            ax2.axvspan(start_date, date, color=regime_colors.get(prev_label, 'gray'), alpha=0.2)
        start_date = date
        prev_label = curr_label
# Final span
ax2.axvspan(start_date, filtered.index[-1], color=regime_colors.get(prev_label, 'gray'), alpha=0.2)
